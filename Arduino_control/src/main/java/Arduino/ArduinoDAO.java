...
public class ArduinoDAO {
	...
	public boolean Arduino_Update2(Arduino1DTO bean) {
		...
		try {
			con = pool.getConnection();
			// 온도 수정 bean.getF03()
			sql = "update arduino03 set s02=?, s05=?, s06=? where s01 = ? and s03 = '00001' ";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, bean.getF02());
			pstmt.setString(2, bean.getF03());
			...
			// 습도 수정 bean.getF04()
			sql = "update arduino03 set s02=?, s05=?, s06=? where s01 = ? and s03 = '00002' ";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, bean.getF02());
			...
			// 조도 수정 bean.getF05()
			sql = "update arduino03 set s02=?, s05=?, s06=? where s01 = ? and s03 = '00003' ";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, bean.getF02());
	...
	public boolean Arduino_Update_Perm(int recnum, String perm) {
		...
		try {
			...
			if(perm.equals("ONOOO")) {
				sql = "update arduino03 set s05='OFFOO' where numb=? and numb > 3";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, recnum);
			} else {
				sql = "update arduino03 set s05='ONOOO' where numb=? and numb > 3";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, recnum);
			}
			int count = pstmt.executeUpdate();
